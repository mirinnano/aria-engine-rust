...
本文テキスト・話者・順序・件数を章ごとに比較し、日付カード、見出し断章、段落の息継ぎ、
沈黙、灰色の `pause ash`、明示ペーシング、中央の自動 `statement`、Day 0／5／10の
...
この改稿版は保存名を`umikaze-v5`へ更新する。初回起動時には同作の
`umikaze-v3`と`umikaze-v4`だけを消去するため、旧版の進行・設定・自動保存は引き継がれない。
...
保存は完成版の`umikaze-v5`と分離した`umikaze-demo-v2`を使う。体験版の初回起動は
...
通常版と体験版の生成先は分離されている。通常版は
`dist/build/full/web`、体験版は`dist/build/demo/web`へ置かれるため、片方の
プレイテストやTauriビルドがもう片方の成果物を上書きしない。
